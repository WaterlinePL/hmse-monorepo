...
from hmse_utils.processing.model_exceptions import HydrusMissingFileError
...
def validate_model(hydrus_archive, validation_dir: str):
    logger.debug(f"Validating model {hydrus_archive.filename}")
    fixed_hydrus_name = local_paths.fix_model_name(hydrus_archive.filename)
    hydrus_path = os.path.join(validation_dir, fixed_hydrus_name)
    ...
    extension = fixed_hydrus_name.split('.')[-1]
    hydrus_id = fixed_hydrus_name.replace(f".{extension}", "")
    return hydrus_id
...
def __fix_hydrus_project(hydrus_base_dir: str):
    for file in os.listdir(hydrus_base_dir):
        ...
        if lowercase_file not in HYDRUS_PROPER_CASING:
            logger.info(f"Skipping case-fixing for file: {file}")
            continue
